...
/**
 * Calculates layouts based on CSS style. See
 * {@link #layoutNode(CSSNode, float, float)}.
 */
public class LayoutEngine {
	...
	private static final int[] leading = { POSITION_TOP, POSITION_BOTTOM, POSITION_LEFT, POSITION_RIGHT, };

	private static final int[] trailing = { POSITION_BOTTOM, POSITION_TOP, POSITION_RIGHT, POSITION_LEFT, };

	private static final int[] pos = { POSITION_TOP, POSITION_BOTTOM, POSITION_LEFT, POSITION_RIGHT, };

	private static final int[] dim = { DIMENSION_HEIGHT, DIMENSION_HEIGHT, DIMENSION_WIDTH, DIMENSION_WIDTH, };

	private static final int[] leadingSpacing = { Spacing.TOP, Spacing.BOTTOM, Spacing.START, Spacing.START };

	private static final int[] trailingSpacing = { Spacing.BOTTOM, Spacing.TOP, Spacing.END, Spacing.END };
	...
	private static void setDimensionFromStyle(CSSNode node, int axis) {
		...
		if (Float.isNaN(node.style.dimensions[dim[axis]]) || node.style.dimensions[dim[axis]] <= 0.0) {
			return;
		}

		// The dimensions can never be smaller than the padding and border
		float maxLayoutDimension = Math.max(boundAxis(node, axis, node.style.dimensions[dim[axis]]),
				node.style.padding.getWithFallback(leadingSpacing[axis], leading[axis]) + node.style.padding.getWithFallback(trailingSpacing[axis], trailing[axis]) + node.style.border.getWithFallback(leadingSpacing[axis], leading[axis]) + node.style.border.getWithFallback(trailingSpacing[axis], trailing[axis]));
	...
	static boolean needsRelayout(CSSNode node, float parentMaxWidth, float parentMaxHeight) {
		return node.isDirty() || !FloatUtil.floatsEqual(node.lastLayout.requestedHeight, node.layout.dimensions[DIMENSION_HEIGHT]) || !FloatUtil.floatsEqual(node.lastLayout.requestedWidth, node.layout.dimensions[DIMENSION_WIDTH]) || !FloatUtil.floatsEqual(node.lastLayout.parentMaxWidth, parentMaxWidth)
				|| !FloatUtil.floatsEqual(node.lastLayout.parentMaxHeight, parentMaxHeight);
	}

	/* package */static void layoutNode(CSSLayoutContext layoutContext, CSSNode node, float parentMaxWidth, float parentMaxHeight, CSSDirection parentDirection) {
	...
	private static void layoutNodeImpl(CSSLayoutContext layoutContext, CSSNode node, float parentMaxWidth, float parentMaxHeight, CSSDirection parentDirection) {
		...
		// Inline immutable values from the target node to avoid excessive
		// method
		// invocations during the layout calculation.
		int childCount = node.getChildCount();
		float paddingAndBorderAxisResolvedRow = ((node.style.padding.getWithFallback(leadingSpacing[resolvedRowAxis], leading[resolvedRowAxis]) + node.style.border.getWithFallback(leadingSpacing[resolvedRowAxis], leading[resolvedRowAxis])) + (node.style.padding.getWithFallback(trailingSpacing[resolvedRowAxis],
				trailing[resolvedRowAxis]) + node.style.border.getWithFallback(trailingSpacing[resolvedRowAxis], trailing[resolvedRowAxis])));
		float paddingAndBorderAxisColumn = ((node.style.padding.getWithFallback(leadingSpacing[CSS_FLEX_DIRECTION_COLUMN], leading[CSS_FLEX_DIRECTION_COLUMN]) + node.style.border.getWithFallback(leadingSpacing[CSS_FLEX_DIRECTION_COLUMN], leading[CSS_FLEX_DIRECTION_COLUMN])) + (node.style.padding.getWithFallback(
		...
		if (isMeasureDefined(node)) {
			...
			height -= ((node.style.padding.getWithFallback(leadingSpacing[CSS_FLEX_DIRECTION_COLUMN], leading[CSS_FLEX_DIRECTION_COLUMN]) + node.style.border.getWithFallback(leadingSpacing[CSS_FLEX_DIRECTION_COLUMN], leading[CSS_FLEX_DIRECTION_COLUMN])) + (node.style.padding.getWithFallback(
					trailingSpacing[CSS_FLEX_DIRECTION_COLUMN], trailing[CSS_FLEX_DIRECTION_COLUMN]) + node.style.border.getWithFallback(trailingSpacing[CSS_FLEX_DIRECTION_COLUMN], trailing[CSS_FLEX_DIRECTION_COLUMN])));

			// We only need to give a dimension for the text if we haven't got
			// any
			// for it computed yet. It can either be from the style attribute or
			// because
			...
			if (isRowUndefined || isColumnUndefined) {
				...
				layoutContext.measureOutput, width, height);
				if (isRowUndefined) {
		...
		float paddingAndBorderAxisMain = ((node.style.padding.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis]) + node.style.border.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis])) + (node.style.padding.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis]) + node.style.border
				.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis])));
		float paddingAndBorderAxisCross = ((node.style.padding.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis]) + node.style.border.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis])) + (node.style.padding.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]) + node.style.border
				.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis])));
		...
		while (endLine < childCount) {
			...
			// mainContentDim is accumulation of the dimensions and margin of
			// all the
			// non flexible children. This will be used in order to either set
			// the
			// dimensions of the node if none already exist, or to compute the
			// remaining space left for the flexible children.
			float mainContentDim = 0;

			// There are three kind of children, non flexible, flexible and
			// absolute.
			// We need to know how many there are in order to distribute the
			// space.
			int flexibleChildrenCount = 0;
			float totalFlexible = 0;
			int nonFlexibleChildrenCount = 0;

			// Use the line loop to position children in the main axis for as
			// long
			...
			// Use the initial line loop to position children in the cross axis
			// for
			// as long as they are relatively positioned with alignment STRETCH
			// or
			// FLEX_START. Children that are immediately stacked in the initial
			// loop
			...
			for (i = startLine; i < childCount; ++i) {
				...
				// Pre-fill cross axis dimensions when the child is using
				// stretch before
				...
				if (alignItem == CSSAlign.STRETCH && child.style.positionType == CSSPositionType.RELATIVE && isCrossDimDefined && !(!Float.isNaN(child.style.dimensions[dim[crossAxis]]) && child.style.dimensions[dim[crossAxis]] >= 0.0)) {
					child.layout.dimensions[dim[crossAxis]] = Math.max(
							boundAxis(child, crossAxis, node.layout.dimensions[dim[crossAxis]] - paddingAndBorderAxisCross - (child.style.margin.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis]) + child.style.margin.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]))),
							// You never want to go smaller than padding
							((child.style.padding.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis]) + child.style.border.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis])) + (child.style.padding.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]) + child.style.border
									.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]))));
				} else if (child.style.positionType == CSSPositionType.ABSOLUTE) {
					// Store a private linked list of absolutely positioned
					// children
					...
					// Pre-fill dimensions when using absolute position and both
					// offsets for the axis are defined (either both
					// left and right or top and bottom).
					for (ii = 0; ii < 2; ii++) {
						axis = (ii != 0) ? CSS_FLEX_DIRECTION_ROW : CSS_FLEX_DIRECTION_COLUMN;
						if ((!Float.isNaN(node.layout.dimensions[dim[axis]]) && node.layout.dimensions[dim[axis]] >= 0.0) && !(!Float.isNaN(child.style.dimensions[dim[axis]]) && child.style.dimensions[dim[axis]] >= 0.0) && !Float.isNaN(child.style.position[leading[axis]])
							...
							child.layout.dimensions[dim[axis]] = Math.max(
									boundAxis(
											...
											node.layout.dimensions[dim[axis]]
													- ((node.style.padding.getWithFallback(leadingSpacing[axis], leading[axis]) + node.style.border.getWithFallback(leadingSpacing[axis], leading[axis])) + (node.style.padding.getWithFallback(trailingSpacing[axis], trailing[axis]) + node.style.border.getWithFallback(
															trailingSpacing[axis], trailing[axis]))) - (child.style.margin.getWithFallback(leadingSpacing[axis], leading[axis]) + child.style.margin.getWithFallback(trailingSpacing[axis], trailing[axis]))
													- (Float.isNaN(child.style.position[leading[axis]]) ? 0 : child.style.position[leading[axis]]) - (Float.isNaN(child.style.position[trailing[axis]]) ? 0 : child.style.position[trailing[axis]])),
									// You never want to go smaller than padding
									((child.style.padding.getWithFallback(leadingSpacing[axis], leading[axis]) + child.style.border.getWithFallback(leadingSpacing[axis], leading[axis])) + (child.style.padding.getWithFallback(trailingSpacing[axis], trailing[axis]) + child.style.border.getWithFallback(
				...
				if (isMainDimDefined && (child.style.positionType == CSSPositionType.RELATIVE && child.style.flex > 0)) {
					...
					// Store a private linked list of flexible children so that
					// we can
					...
					// Even if we don't know its exact size yet, we already know
					// the padding,
					// border and margin. We'll use this partial information,
					// which represents
					// the smallest possible size for the child, to compute the
					// remaining
					// available space.
					nextContentDim = ((child.style.padding.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis]) + child.style.border.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis])) + (child.style.padding.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis]) + child.style.border
							.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis]))) + (child.style.margin.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis]) + child.style.margin.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis]));
					...
					// This is the main recursive call. We layout non flexible
					// children.
					if (alreadyComputedNextLayout == 0) {
						layoutNode(layoutContext, child, maxWidth, maxHeight, direction);
					}

					// Absolute positioned elements do not take part of the
					// layout, so we
					// don't use them to compute mainContentDim
					if (child.style.positionType == CSSPositionType.RELATIVE) {
						nonFlexibleChildrenCount++;
						// At this point we know the final size and margin of
						// the element.
				...
				// The element we are about to add would make us go to the next
				// line
				if (isNodeFlexWrap && isMainDimDefined && mainContentDim + nextContentDim > definedMainDim &&
				// If there's only one element, then it's bigger than the
				// content
				...
				// Disable simple stacking in the main axis for the current line
				// as
				// we found a non-trivial child. The remaining children will be
				// laid out
				// in <Loop C>.
				...
				// Disable simple stacking in the cross axis for the current
				// line as
				// we found a non-trivial child. The remaining children will be
				// laid out
				// in <Loop D>.
				if (isSimpleStackCross && (child.style.positionType != CSSPositionType.RELATIVE || (alignItem != CSSAlign.STRETCH && alignItem != CSSAlign.FLEX_START) || !(!Float.isNaN(child.layout.dimensions[dim[crossAxis]]) && child.layout.dimensions[dim[crossAxis]] >= 0.0))) {
			...
			// If there are flexible children in the mix, they are going to fill
			// the
			// remaining space
			if (flexibleChildrenCount != 0) {
				...
				// If the flex share of remaining space doesn't meet min/max
				// bounds,
				// remove this child from flex calculations.
				currentFlexChild = firstFlexChild;
				while (currentFlexChild != null) {
					baseMainDim = flexibleMainDim
							* currentFlexChild.style.flex
							+ ((currentFlexChild.style.padding.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis]) + currentFlexChild.style.border.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis])) + (currentFlexChild.style.padding.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis]) + currentFlexChild.style.border
									.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis])));
				...
				// The non flexible children can overflow the container, in this
				// case
				...
				currentFlexChild = firstFlexChild;
				while (currentFlexChild != null) {
					// At this point we know the final size of the element in
					// the main
					// dimension
					currentFlexChild.layout.dimensions[dim[mainAxis]] = boundAxis(
							...
							flexibleMainDim
									* currentFlexChild.style.flex
									+ ((currentFlexChild.style.padding.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis]) + currentFlexChild.style.border.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis])) + (currentFlexChild.style.padding.getWithFallback(trailingSpacing[mainAxis],
											trailing[mainAxis]) + currentFlexChild.style.border.getWithFallback(trailingSpacing[mainAxis], trailing[mainAxis]))));
					...
					// And we recursively call the layout algorithm for this
					// child
				...
				// We use justifyContent to figure out how to allocate the
				// remaining
				// space available
			...
			// <Loop C> Position elements in the main axis and compute
			// dimensions

			// At this point, all the children have their dimensions set. We
			// need to
			// find their position. In order to do that, we accumulate data in
			// variables that are also useful to compute the total dimensions of
			// the
			// container!
			...
			for (i = firstComplexMain; i < endLine; ++i) {
				...
				if (child.style.positionType == CSSPositionType.ABSOLUTE && !Float.isNaN(child.style.position[leading[mainAxis]])) {
					...
					// being
					// defined, we override the position to whatever the user
					// said
					// (and margin/border).
					child.layout.position[pos[mainAxis]] = (Float.isNaN(child.style.position[leading[mainAxis]]) ? 0 : child.style.position[leading[mainAxis]]) + node.style.border.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis])
							+ child.style.margin.getWithFallback(leadingSpacing[mainAxis], leading[mainAxis]);
				} else {
					// If the child is position absolute (without top/left) or
					// relative,
					...
					// variables
					// We only need to do that for relative elements. Absolute
					// elements
					...
					if (child.style.positionType == CSSPositionType.RELATIVE) {
						...
						// The cross dimension is the max of the elements
						// dimension since there
			...
			for (i = firstComplexCross; i < endLine; ++i) {
				...
				if (child.style.positionType == CSSPositionType.ABSOLUTE && !Float.isNaN(child.style.position[leading[crossAxis]])) {
					...
					// top/left/bottom/right being set, we override all the
					// previously
					// computed positions to set it correctly.
					child.layout.position[pos[crossAxis]] = (Float.isNaN(child.style.position[leading[crossAxis]]) ? 0 : child.style.position[leading[crossAxis]]) + node.style.border.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis])
					...
					// (parent) or
					// alignSelf (child) in order to determine the position in
					// the cross axis
					if (child.style.positionType == CSSPositionType.RELATIVE) {
						/* eslint-disable */
						// This variable is intentionally re-defined as the code
						// is transpiled to a block scope language
						CSSAlign alignItem = getAlignItem(node, child);
						/* eslint-enable */
						if (alignItem == CSSAlign.STRETCH) {
							// You can only stretch if the dimension has not
							// already been set
							// previously.
							if (!(!Float.isNaN(child.layout.dimensions[dim[crossAxis]]) && child.layout.dimensions[dim[crossAxis]] >= 0.0)) {
								child.layout.dimensions[dim[crossAxis]] = Math.max(
										boundAxis(child, crossAxis, containerCrossAxis - paddingAndBorderAxisCross - (child.style.margin.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis]) + child.style.margin.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]))),
										// You never want to go smaller than
										// padding
										((child.style.padding.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis]) + child.style.border.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis])) + (child.style.padding.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]) + child.style.border
												.getWithFallback(trailingSpacing[crossAxis], trailing[crossAxis]))));
							}
						} else if (alignItem != CSSAlign.FLEX_START) {
							// The remaining space between the parent
							// dimensions+padding and child
							// dimensions+margin.
		...
		//
		// Note that we could probably remove <Loop D> and handle the one line
		// case
		// here too, but for the moment this is safer since it won't interfere
		// with
			...
			for (i = 0; i < linesCount; ++i) {
				...
				for (ii = startIndex; ii < endIndex; ++ii) {
					...
					if (alignContentAlignItem == CSSAlign.FLEX_START) {
						...
						child.layout.position[pos[crossAxis]] = currentLead + child.style.margin.getWithFallback(leadingSpacing[crossAxis], leading[crossAxis]);
						// TODO(prenaux): Correctly set the height of items with
						// undefined
		...
		if (!isMainDimDefined) {
			...
			// dimension
					...
					// We can never assign a width smaller than the padding and
					// borders
		...
		while (currentAbsoluteChild != null) {
			...
			// for
			// the axis are defined (either both left and right or top and
			// bottom).
			for (ii = 0; ii < 2; ii++) {
				axis = (ii != 0) ? CSS_FLEX_DIRECTION_ROW : CSS_FLEX_DIRECTION_COLUMN;

				if ((!Float.isNaN(node.layout.dimensions[dim[axis]]) && node.layout.dimensions[dim[axis]] >= 0.0) && !(!Float.isNaN(currentAbsoluteChild.style.dimensions[dim[axis]]) && currentAbsoluteChild.style.dimensions[dim[axis]] >= 0.0) && !Float.isNaN(currentAbsoluteChild.style.position[leading[axis]])
						&& !Float.isNaN(currentAbsoluteChild.style.position[trailing[axis]])) {
					currentAbsoluteChild.layout.dimensions[dim[axis]] = Math
							.max(boundAxis(currentAbsoluteChild, axis,
									node.layout.dimensions[dim[axis]] - (node.style.border.getWithFallback(leadingSpacing[axis], leading[axis]) + node.style.border.getWithFallback(trailingSpacing[axis], trailing[axis]))
											...
											- (Float.isNaN(currentAbsoluteChild.style.position[leading[axis]]) ? 0 : currentAbsoluteChild.style.position[leading[axis]]) - (Float.isNaN(currentAbsoluteChild.style.position[trailing[axis]]) ? 0 : currentAbsoluteChild.style.position[trailing[axis]])),
							// You never want to go smaller than padding
									((currentAbsoluteChild.style.padding.getWithFallback(leadingSpacing[axis], leading[axis]) + currentAbsoluteChild.style.border.getWithFallback(leadingSpacing[axis], leading[axis])) + (currentAbsoluteChild.style.padding.getWithFallback(trailingSpacing[axis], trailing[axis]) + currentAbsoluteChild.style.border
											.getWithFallback(trailingSpacing[axis], trailing[axis]))));
				}
